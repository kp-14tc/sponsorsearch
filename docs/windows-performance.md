...
Docker's WSL2 backend uses the WSL utility VM's memory, CPU, and swap configuration. Do not look for the Hyper-V CPU and RAM sliders or switch away from WSL2 to obtain them. **Docker Desktop → Settings → General → Use the WSL 2 based engine** should remain enabled. **Settings → Resources → WSL Integration** selects distribution integration; it does not allocate native Ollama resources. [Docker Desktop resource settings](https://docs.docker.com/desktop/settings-and-maintenance/settings/)
